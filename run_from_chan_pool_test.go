...
func TestRunFromChanPool(t *testing.T) {
	...
	for tcName, tc := range testCases {
		...
		t.Run(tcName, func(t *testing.T) {
			...
			resCh, err := merec.RunWorkerPool(ctx, tc.givenIn, stabCall(10*time.Microsecond), balancedWorkload(cap(tc.givenIn)))
...
func TestRunFromPool_Errors(t *testing.T) {
	...
	for tcName, tc := range testCases {
		...
		t.Run(tcName, func(t *testing.T) {
			...
			resCh, err := merec.RunWorkerPool(ctx, tc.givenIn(), stabCall(10*time.Millisecond), balancedWorkload(cap(tc.givenIn())))
			require.NoError(t, err)
...
func TestRunFromPool_ValidationFail(t *testing.T) {
	...
	for tcName, tc := range testCases {
		...
		t.Run(tcName, func(t *testing.T) {
			...
			resCh, err := merec.RunWorkerPool(tc.givenCtx, tc.givenIn(), tc.givenCall, balancedWorkload(cap(tc.givenIn())))
...
func TestRunFromPool_WithOptions(t *testing.T) {
	...
	for tcName, tc := range testCases {
		...
		t.Run(tcName, func(t *testing.T) {
			...
			resCh, err := merec.RunWorkerPool(ctx, tc.givenIn, stabCall(time.Second), balancedWorkload(cap(tc.givenIn)), tc.givenOptions...)
